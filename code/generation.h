...
//            delay = 5;
            ...
            while (!s.empty() && (priority.find(s.top()) == priority.end() || priority.at(s.top()) > priority.at(x.second))) {
                vec.push_back(s.top());
                s.pop();
            }
            s.push(x.second);
...
//                throw std::string("GenerationError: Unbalanced braces");
                ...
                if (!s.empty()) {
                    ...
                    if (functions.find(isFunc) != functions.end()) {
                        s.pop();
...
std::map<std::string, std::tuple<std::string, std::string, bool, std::string>> tidID) {
    ...
    for (int i = 0; i < vec.size(); ++i) {
        if (vec[i] == "=") {
            ...
            std::cout << val;
        } else if (functions.find(vec[i]) != functions.end()) {
            if (vec[i] == "return") {
                std::string el2 = handler.top();
                if (tidID.find(el2) != tidID.end()) {
                    auto res = tidID[el2];
